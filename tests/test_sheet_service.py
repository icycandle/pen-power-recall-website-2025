...
class TestSheetService(unittest.TestCase):
    ...
    def test_init_with_file_credentials(
        self, mock_exists, mock_credentials, mock_gspread
    ):
        """測試使用檔案憑證初始化"""
        # 模擬憑證
        mock_creds = MagicMock()
    ...
    def test_get_sheet_data(self, mock_credentials, mock_gspread):
        ...
        with patch.dict(os.environ, {"GOOGLE_CREDENTIALS": json.dumps(test_creds)}):
            ...
            self.assertEqual(
                result.rows[0], ["測試標題1", "測試作者1", "https://example.com/1"]
            )
            self.assertEqual(
                result.rows[1], ["測試標題2", "測試作者2", "https://example.com/2"]
            )
