...
class HtmlGenerator:
    ...
    def generate_site(self, data: SheetData, output_dir: str) -> None:
        ...
        # 映射過濾後的欄位索引
        new_indices = self._update_indices_after_filter(orig_indices, data, filtered_data)
    ...
    def _update_indices_after_filter(self, orig_indices: dict, orig_data: SheetData, filtered_data: SheetData) -> dict:
        """
        在過濾後更新欄位索引
    ...
    def _filter_sensitive_data(self, data: SheetData) -> SheetData:
        ...
        for i, header in enumerate(data.headers):
            header_lower = header.lower()
            if "電子郵件" in header_lower or "email" in header_lower or "mail" in header_lower:
    ...
    def _generate_index_page(self, data: SheetData, output_dir: str, indices: dict) -> None:
        """
        生成首頁 HTML 檔案
